...
class AmqpService
{
     ...
     * @param array $publishOptions Publishing options (mandatory)
     * @param string $connectionName The connection name to use
     * @return void
     */
    public static function publishToQueue(
        $payload,
        string $queue,
        array $properties = [],
        array $publishOptions = [],
        ...
        self::publish($payload, '', $queue, $properties, $publishOptions, $connectionName);
    }
    /**
     * Helper method for services to reply to RPC requests
     * This handles the correlation ID and reply-to queue automatically
     *
     * @param AMQPMessage $requestMessage The original request message
     * @param mixed $responsePayload The response payload (will be JSON encoded)
     * @param array $messageProperties Additional message properties for the response
     * @param string $connectionName The connection name to use
     * @return void
     * @throws \RuntimeException If the request message doesn't have reply-to or correlation-id
     * @throws \JsonException If response payload can't be JSON encoded
     */
    public static function replyToRequest(
        AMQPMessage $requestMessage,
        $responsePayload,
        array $messageProperties = [],
        string $connectionName = 'default'
    ): void {
        ...
        // Validate that this is a request that expects a reply
        $replyTo = $requestMessage->get('reply_to');
        $correlationId = $requestMessage->get('correlation_id');

        if (empty($replyTo)) {
            throw new \RuntimeException('Request message does not have a reply_to property');
        }

        if (empty($correlationId)) {
            throw new \RuntimeException('Request message does not have a correlation_id property');
        }

        // Prepare response message properties
        $responseProperties = array_merge(
            self::$defaultMessageProperties,
            $messageProperties,
            [
                'correlation_id' => $correlationId,
                // Don't set reply_to on response messages
            ]
        );

        $json = json_encode($responsePayload, JSON_THROW_ON_ERROR);
        $responseMessage = new AMQPMessage($json, $responseProperties);

        self::execute(function (AMQPChannel $channel) use ($responseMessage, $replyTo, $correlationId) {
            // Publish to the reply queue (usually amq.rabbitmq.reply-to)
            $channel->basic_publish($responseMessage, '', $replyTo);

            Log::debug("Sent RPC reply", [
                'reply_to' => $replyTo,
                'correlation_id' => $correlationId,
                'payload_size' => strlen($responseMessage->getBody())
            ]);
        }, $connectionName, 'reply_' . $correlationId);
    }

    /**
     * Publish a message and wait for a response using RabbitMQ Direct Reply-To pattern
     *
     * @param mixed $payload The payload to publish (will be JSON encoded)
     * @param string $exchange The exchange name
     * @param string $routingKey The routing key
     * @param array $options RPC options (timeout, mandatory, message_properties)
     * @param string $connectionName The connection name to use
     * @return mixed The response payload (decoded from JSON if applicable)
     * @throws \RuntimeException If timeout occurs or operation fails
     * @throws \JsonException If response JSON is invalid
     */
    public static function publishAndWaitForReply(
        $payload,
        string $exchange,
        string $routingKey,
        array $options = [],
        ...
        $options = array_merge(self::$defaultRpcOptions, $options);
        $messageProperties = array_merge(
            self::$defaultMessageProperties,
            $options['message_properties'] ?? [],
            [
                'reply_to' => 'amq.rabbitmq.reply-to',  // Direct reply-to queue
        ...
        return self::execute(function (AMQPChannel $channel) use ($message, $exchange, $routingKey, $options) {
            $correlationId = $message->get('correlation_id');
            $responseReceived = false;
            $responseData = null;
            $responseError = null;

            Log::debug("Starting RPC call", [
                'exchange' => $exchange,
            ...
            // Set up consumer for the direct reply-to queue
            $replyConsumerTag = 'reply_' . $correlationId;
            $channel->basic_consume(
                queue: 'amq.rabbitmq.reply-to',
                consumer_tag: $replyConsumerTag,
                no_local: false,
                no_ack: true,  // Auto-ack reply messages
                exclusive: false,
                nowait: false,
                callback: function (AMQPMessage $replyMessage) use ($correlationId, &$responseReceived, &$responseData, &$responseError, $channel, $replyConsumerTag) {
                    try {
                        // Check if this reply is for our request
                        if ($replyMessage->get('correlation_id') !== $correlationId) {
                            Log::debug("Received reply for different correlation ID", [
                                'expected' => $correlationId,
                                'received' => $replyMessage->get('correlation_id')
                            ]);
                            return;
                        }
                        ...
                        Log::debug("Received RPC response", [
                            'correlation_id' => $correlationId,
                            'content_type' => $replyMessage->get('content_type')
                        ]);

                        // Parse response based on content type
                        $body = $replyMessage->getBody();
                        $contentType = $replyMessage->get('content_type');

                        if (true || $contentType === 'application/json') {
                            try {
                                $responseData = json_decode($body, true, 512, JSON_THROW_ON_ERROR);
                            } catch (\JsonException $e) {
                                $responseError = new \JsonException(
                                    "Failed to decode RPC response JSON: " . $e->getMessage(),
                                    $e->getCode(),
                                    $e
                                );
                                Log::error("Failed to decode RPC response JSON", [
                                    'correlation_id' => $correlationId,
                                    'error' => $e->getMessage(),
                                    'body' => $body
                                ]);
                        ...
                        // Cancel the consumer to stop waiting
                        $channel->basic_cancel($replyConsumerTag);
                    } catch (\Exception $e) {
                        $responseError = $e;
                        $responseReceived = true;
                        $channel->basic_cancel($replyConsumerTag);

                        Log::error("Error processing RPC response", [
                            'correlation_id' => $correlationId,
                            'error' => $e->getMessage()
                        ]);
                    }
            ...
            $channel->basic_publish($message, $exchange, $routingKey, $options['mandatory']);

            Log::debug("Published RPC request", [
                'exchange' => $exchange,
                'routing_key' => $routingKey,
                'correlation_id' => $correlationId
            ]);
            ...
            while (!$responseReceived && !self::$shouldShutdown) {
                $elapsed = time() - $startTime;

                if ($elapsed >= $timeout) {
                    // Cancel consumer and throw timeout exception
                    try {
                        $channel->basic_cancel($replyConsumerTag);
                    } catch (\Exception $e) {
                    ...
                    Log::warning("RPC call timed out", [
                        'correlation_id' => $correlationId,
                        'timeout' => $timeout,
                        'elapsed' => $elapsed
                    ]);

                    throw new \RuntimeException("RPC call timed out after {$timeout} seconds");
                }

                try {
                    // Wait for messages with a short timeout to allow periodic timeout checks
                    $remainingTimeout = min(1, $timeout - $elapsed);
                    $channel->wait(null, false, $remainingTimeout);
            ...
            if (self::$shouldShutdown) {
                ...
                throw new \RuntimeException("RPC call aborted due to shutdown signal");
            }

            // Check for response processing errors
            ...
            return $responseData;
        }, $connectionName, 'rpc_' . uniqid());
    }

    /**
     * Publish a message to a queue and wait for a response using RabbitMQ Direct Reply-To pattern
     *
     * @param mixed $payload The payload to publish (will be JSON encoded)
     * @param string $queue The queue name
     * @param array $options RPC options (timeout, message_properties)
     * @param string $connectionName The connection name to use
     * @return mixed The response payload (decoded from JSON if applicable)
     * @throws \RuntimeException If timeout occurs or operation fails
     * @throws \JsonException If response JSON is invalid
     */
    public static function publishToQueueAndWaitForReply(
        $payload,
        string $queue,
        array $options = [],
        string $connectionName = 'default'
    ) {
        return self::publishAndWaitForReply($payload, '', $queue, $options, $connectionName);
    }
        ...
        self::executeConsumer(function (AMQPChannel $channel) use ($queue, $callback, $options) {
            ...
            $wrappedCallback = function (AMQPMessage $message) use ($callback, $options, $queue, $channel) {
                try {
                    ...
                    Log::info("Got message with routing key ". $message->getRoutingKey() . ", content_type ". $message->get('content_type') . " and body '''".$body."'''");
                    ...
                    if (true || $contentType === 'application/json') {
                        try {
                            ...
                            $message->nack(requeue: true, multiple: false);
                            return;
                        }
                    } else {
                    ...
                    Log::debug("Consuming message from queue", [
                        'queue' => $queue,
                    ...
                    $message->nack(requeue: true, multiple: true);
                }
            };
